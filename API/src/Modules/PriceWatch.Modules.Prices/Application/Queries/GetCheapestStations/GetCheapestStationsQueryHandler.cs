...
internal sealed class GetCheapestStationsQueryHandler
    : IRequestHandler<GetCheapestStationsQuery, Result<IReadOnlyList<StationPriceDto>>>
{
    private readonly IStationPriceRepository _repository;

    public GetCheapestStationsQueryHandler(IStationPriceRepository repository)
    {
        _repository = repository;
    }

    public async Task<Result<IReadOnlyList<StationPriceDto>>> Handle(
        GetCheapestStationsQuery request,
        CancellationToken cancellationToken)
    {
        var stations = await _repository.GetCheapestByFuelTypeAsync(
            request.FuelType,
            request.Limit,
            cancellationToken);
